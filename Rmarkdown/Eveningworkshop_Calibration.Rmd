...
# 
# month_validate <- function(numeric_month, dataframe) {
#   
#   intermediate_df <- dataframe %>% group_by(wy, month) %>%
#     summarize(model=sum(model), obs=sum(obs)) 
#   
#   month_df <- subset(intermediate_df, month==numeric_month)
#   
#   results = cor(month_df$model, month_df$obs)
#   
#   return(results)
# }

## finds the error for the min rainfall for the summer months
# summer_validate <- function(dataframe){
# intermediate <- dataframe %>% group_by(wy, month) %>%
#     summarize(model=sum(model), obs=sum(obs)) %>%
#   filter(month %in% c(6,7,8,9)) %>% 
#   group_by(wy) %>%
#   summarize(min_obs = min(obs), min_mod = min(model)) %>%
#   mutate(error=(abs(min_obs-min_mod)/min_obs))
#          
# 
# error = mean(intermediate$error)
# }
# 
# ## finds the error for the max rainfall for the summer months
# winter_validate <- function(dataframe){
# intermediate <- dataframe %>% group_by(wy, month) %>%
#     summarize(model=sum(model), obs=sum(obs)) %>%
#   filter(month %in% c(12,1,2,3)) %>% 
#   group_by(wy) %>%
#   summarize(max_obs = max(obs), max_mod = max(model)) %>%
#   mutate(error=(abs(max_obs-max_mod)/max_obs))
#          
# 
# error = mean(intermediate$error)
# }
...
combine_metrics(m=sager$model,o=sager$obs, month=sager$month, day=sager$day, year=sager$year, wy=sager$wy)
